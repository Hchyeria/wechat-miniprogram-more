...
Component({
  ...
  methods: {
    ...
    switchChange(e){
      let { value } = e.detail
      if (value){
        this.triggerEvent('changeOnlySchool', { id: this.data.sortMode, isOnlySchool: 1 })
        this.setData({
          isOnlySchool: 1
        })
      }
      else {
        this.triggerEvent('changeOnlySchool', { id: this.data.sortMode, isOnlySchool: 0 })
        this.setData({
          isOnlySchool: 0
        })
      }
    ...
    chooseMode(e) {
      ...
      this.triggerEvent('chooseMode', { id })
    }
  }
})
